...
// ---- JJF（9月予選→10月決勝） ----
test('JJF: 予選turn/決勝turnの判定', () => {
  assert.deepStrictEqual(DT.DATA.JJF.qualifierTurns.map(t => !!DT.contest.jjfQualifierForTurn(t)), [true, true, true, true]);
  assert.deepStrictEqual(DT.DATA.JJF.finalTurns.map(t => !!DT.contest.jjfFinalForTurn(t)), [true, true, true, true]);
  assert.strictEqual(DT.contest.jjfQualifierForTurn(5), null);
  assert.strictEqual(DT.contest.jjfFinalForTurn(5), null);
  // 予選(9月)の翌月が決勝(10月)
  DT.DATA.JJF.qualifierTurns.forEach((qt, i) => assert.strictEqual(DT.DATA.JJF.finalTurns[i], qt + 1));
});

test('JJF予選: バランス平均/最低でtier判定（sure/half/none）', () => {
  const mk = (v) => { const s = allFifty(); DT.DATA.GENRES.forEach(g => DT.DATA.METHODS.forEach(m => s.skills[g.id][m.id] = v)); s.composition = v; return s; };
  assert.strictEqual(DT.contest.jjfQualify(mk(65), () => 0.9).tier, 'sure'); // avg65/min65 >= 60/50
  assert.strictEqual(DT.contest.jjfQualify(mk(55), () => 0.9).tier, 'half'); // 60未満だがpassHalf(50/40)以上
  assert.strictEqual(DT.contest.jjfQualify(mk(45), () => 0.9).tier, 'none'); // passHalf未満
  // sureは乱数に関わらず突破、halfは50%（rng<0.5で突破）
  assert.strictEqual(DT.contest.jjfQualify(mk(65), () => 0.99).passed, true);
  assert.strictEqual(DT.contest.jjfQualify(mk(55), () => 0.4).passed, true);
  assert.strictEqual(DT.contest.jjfQualify(mk(55), () => 0.6).passed, false);
  // バランスが悪い（1マスだけ低い）とminが下がりtierが落ちる
  const lop = mk(65); lop.skills.h1d.difficulty = 10; // h1d習熟が下がる
  assert.notStrictEqual(DT.contest.jjfQualify(lop, () => 0.9).tier, 'sure');
});

test('JJF決勝: 10人・上位3名のみ追加ポイント・resultsに記録', () => {
  const jf = { turn: 43, type: 'jjf', name: '4年 JJF決勝' };
  const s = allFifty();
  const before = s.results.length;
  const r = DT.contest.runJjfFinal(s, jf, () => 0.5)[0];
  assert.strictEqual(r.entrants, 10);
  assert.strictEqual(r.division, 'overall');
  assert.strictEqual(s.results.length, before + 1); // resultsに記録される
  // 上位3名なら追加ポイント、それ以外は0
  if (r.rank <= 3) assert.strictEqual(r.points, DT.DATA.JJF.finalRankPoints[r.rank - 1]);
