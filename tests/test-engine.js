...
test('computeSlotGain経由: motivation80以上（絶好調）で成功枠ゲインに+1、失敗枠には乗らない', () => {
  ...
  assert.strictEqual(r.results[0].gain, 7); // round(4*1*1*1.5[1年目])=6 +hotBonus(1) = 7
  assert.strictEqual(s.skills.v1d.difficulty, 17);

  const sFail = base();
  sFail.motivation = 80;
  // 失敗が起きるのは新技開発(novelty)のみ（高難度技は失敗しないため失敗枠の検証はnoveltyで行う）
  const rFail = DT.engine.applyTraining(sFail, [{ genre: 'v1d', method: 'novelty' }], () => 0.2); // 失敗
  assert.strictEqual(rFail.results[0].tier, '失敗');
  assert.strictEqual(sFail.skills.v1d.novelty, 10); // 失敗はゲイン0のまま、hotBonusも乗らない

  const sBelow = base();
  sBelow.motivation = 79;
  const rBelow = DT.engine.applyTraining(sBelow, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(rBelow.results[0].gain, 6); // hotLine未満なのでボーナスなし（1年目1.5倍のみ）
});

test('applyTraining: 失敗は新技開発(novelty)のみ。高難度技/反復練習/ルーチンは低乱数でも失敗しない', () => {
  ['difficulty', 'control'].forEach(method => {
    const s = base();
    const r = DT.engine.applyTraining(s, [{ genre: 'h1d', method: method }], () => 0.15); // 失敗帯の乱数
    assert.notStrictEqual(r.results[0].tier, '失敗', method + 'は失敗しないはず');
  });
  const sr = base();
  const rr = DT.engine.applyTraining(sr, ['routine'], () => 0.15);
  assert.notStrictEqual(rr.results[0].tier, '失敗', 'ルーチンは失敗しないはず');
  const sn = base();
  const rn = DT.engine.applyTraining(sn, [{ genre: 'h1d', method: 'novelty' }], () => 0.15);
  assert.strictEqual(rn.results[0].tier, '失敗', '新技開発は失敗し得る');
});

test('applyTraining: 屋外練習デバフ(outdoorTurns>0)はゲイン半減・1セッションで消費', () => {
  const s = base();
  s.outdoorTurns = 1;
  const r = DT.engine.applyTraining(s, [{ genre: 'h1d', method: 'control' }], () => 0.3); // 成功
  // 通常 round(4×1×1×1.5[1年目])=6 → デバフで round(4×1×1×1.5×0.5)=3 → 10+3
  assert.strictEqual(r.results[0].gain, 3);
  assert.strictEqual(s.skills.h1d.control, 13);
  assert.strictEqual(s.outdoorTurns, 0); // 1セッションで消費
  assert.ok(r.messages.some(m => m.includes('屋外練習')));
  const r2 = DT.engine.applyTraining(s, [{ genre: 'h1d', method: 'control' }], () => 0.3);
  assert.strictEqual(r2.results[0].gain, 6); // 消費後は通常ゲイン
});
...
// 全マス10・turn1(1年目)・rng固定0.3（全枠成功）: [{h1d,control}]×4
// 各枠: gain=round(4×1.0×growthMult(現在値)×1.5[1年目])=6。growthMultは10→16→22→28はいずれも<40なので常に1.0。
// → skills.h1d.control = 10 + 6×4 = 34
// fatigue = SLOTS.fatigue.control(3) × 4 = 12（1年目ボーナスはゲインのみ、疲労は不変）
...
test('applyTraining: 検算ピン 全枠{h1d,control}・rng0.3固定', () => {
  ...
  assert.strictEqual(s.skills.h1d.control, 34);
...
test('applyTraining: 基本ゲイン（単枠・成功、マス以外のジャンル/methodは不変）', () => {
  ...
  assert.strictEqual(s.skills.h1d.control, 16); // 10 + round(4*1*1*1.5[1年目])=6
  ...
  assert.strictEqual(r.results[0].tier, '成功');
  assert.strictEqual(r.results[0].gain, 6);
  assert.ok(r.messages[0].includes('1DH'));
  assert.ok(r.messages[0].includes('操作安定度 +6') || r.messages[0].includes('+6'));
});
...
test('applyTraining: routine枠はcompositionのみ加算・skillsは不変', () => {
  ...
  assert.strictEqual(s.composition, 13); // 10 + round(2*1*1*1.5[1年目])=3
  ...
  assert.ok(r.messages[0].includes('演技構成 +3'));
});
...
test('applyTraining: 大会月のroutineはブースト、difficultyはペナルティ+追加疲労、controlは倍化', () => {
  const sR = mk(5); // 1年OIDC月（1年目）
  DT.engine.applyTraining(sR, ['routine'], () => 0.3);
  assert.strictEqual(sR.composition, 15); // round(2×1×1×1.5[大会routine]×1.5[1年目])=round(4.5)=5 → 10+5
  const sD = mk(5);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.skills.v1d.difficulty, 13); // round(4×1×1×0.5[大会difficulty]×1.5[1年目])=3 → 10+3
  ...
  assert.strictEqual(sC.skills.v1d.control, 22); // round(4×1×1×2.0[大会control]×1.5[1年目])=12 → 10+12
});

test('applyTraining: 大会月の高難度技は失敗せず普通に読み替え・追加疲労はtier非依存で乗る', () => {
  const s = mk(5);
  // 本来は失敗帯の乱数だが、高難度技(difficulty)は失敗しないため「普通」に読み替えられる
  const r = DT.engine.applyTraining(s, [{ genre: 'v1d', method: 'difficulty' }], () => 0.15);
  assert.strictEqual(r.results[0].tier, '普通');
  assert.strictEqual(s.skills.v1d.difficulty, 12); // round(4×0.5[普通]×0.5[大会月]×1.5[1年目])=2 → 10+2
  assert.strictEqual(s.fatigue, 6); // 5 + extraFatiguePerSlot(1)（大会月difficultyの追加疲労はtier非依存）
});

test('applyTraining: 練習会月はroutine/noveltyの伸びが1.5倍、他は不変', () => {
  const sR = mk(3); // 練習会月（1年目）
  DT.engine.applyTraining(sR, ['routine'], () => 0.3);
  assert.strictEqual(sR.composition, 15); // round(2×1×1×1.5[練習会]×1.5[1年目])=round(4.5)=5 → 10+5
  const sN = mk(3);
  DT.engine.applyTraining(sN, [{ genre: 'v1d', method: 'novelty' }], () => 0.3);
  assert.strictEqual(sN.skills.v1d.novelty, 19); // round(4×1×1×1.5[練習会]×1.5[1年目])=9 → 10+9
  const sD = mk(3);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.skills.v1d.difficulty, 16); // 練習会対象外: round(4×1×1×1.5[1年目])=6 → 10+6
});

// 二重丸めバグ回帰テスト（2026-07-07 実プレイ報告）: 練習会でroutineの成功と普通が
// どちらも+2に潰れていた。倍率を掛けてから最後に1回だけ丸めることでtierの差を保つ。
// 成功: round(2×1.0×1.0×1.5[練習会]×1.5[1年目])=round(4.5)=5 / 普通: round(2×0.5×1.5×1.5)=round(2.25)=2
test('applyTraining: 練習会月でもroutineの成功(+5)と普通(+2)は区別される（二重丸め回帰）', () => {
  const sSuccess = mk(3); // 練習会月・rng0.3=成功
  DT.engine.applyTraining(sSuccess, ['routine'], () => 0.3);
  assert.strictEqual(sSuccess.composition, 15); // 10 + 5

  const sNormal = mk(3); // 練習会月・rng0.7=普通
  DT.engine.applyTraining(sNormal, ['routine'], () => 0.7);
  assert.strictEqual(sNormal.composition, 12); // 10 + 2（普通は成功より少ない）
});
...
test('applyTraining: 特別指導解放で成功枠ごとに+1（タイミング補正の後・フラット加算）', () => {
  ...
  DT.engine.applyTraining(s, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(s.skills.v1d.difficulty, 17); // round(4×1×1×1.5[1年目])=6 +1 = 7 → 10+7
  const sFail = base();
  sFail.specialUnlocked = true;
  DT.engine.applyTraining(sFail, [{ genre: 'v1d', method: 'novelty' }], () => 0.15); // 失敗（失敗するのは新技開発のみ）
  assert.strictEqual(sFail.skills.v1d.novelty, 10); // 失敗は+1も乗らない

  const sMeetup = mk(3);
  sMeetup.specialUnlocked = true;
  DT.engine.applyTraining(sMeetup, [{ genre: 'v1d', method: 'novelty' }], () => 0.3);
  assert.strictEqual(sMeetup.skills.v1d.novelty, 20); // round(4×1.5[練習会]×1.5[1年目])=9 +1 = 10 → 10+10
});
...
test('applyTraining: routineのみの月は疲労・怪我リスクが回復する（tier非依存）', () => {
  ...
  // tier非依存の確認: tierを混在させても、fatigue/riskの最終値は変わらない
  // （ルーチン構成は失敗しないため、大成功/成功/普通の混在で確認する）
  const s2 = base();
  s2.fatigue = 30;
  s2.injuryRisk = 20;
  const seq = [0.05, 0.3, 0.7, 0.3]; // 大成功/成功/普通/成功
  let i = 0;
  const rng = () => seq[i++];
  const r2 = DT.engine.applyTraining(s2, ['routine', 'routine', 'routine', 'routine'], rng);
  assert.notStrictEqual(r2.results[0].tier, '失敗'); // ルーチンは失敗しない
...
test('applyTraining: 大成功でやる気+8（大幅）、通常成功以下は不変', () => {
  const s = base();
  DT.engine.applyTraining(s, [{ genre: 'v1d', method: 'difficulty' }], () => 0.0); // 大成功
  assert.strictEqual(s.motivation, 58); // 50 + greatBonus(8)
});

test('applyTraining: 新技開発は成功でやる気+3（新技を覚えた高揚）、大成功はnoveltyGreatフラグ＋大幅', () => {
  const sS = base();
  DT.engine.applyTraining(sS, [{ genre: 'v1d', method: 'novelty' }], () => 0.3); // 成功
  assert.strictEqual(sS.motivation, 53); // 50 + noveltySuccessBonus(3)
  const sG = base();
  const r = DT.engine.applyTraining(sG, [{ genre: 'v1d', method: 'novelty' }], () => 0.0); // 大成功
  assert.strictEqual(sG.motivation, 58); // 50 + greatBonus(8)（noveltySuccessBonusは大成功では乗らない）
  assert.strictEqual(r.noveltyGreat, true);
  // 高難度技の成功ではやる気は動かない（noveltyのみ成功高揚）
  const sD = base();
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3); // 成功
  assert.strictEqual(sD.motivation, 50);
});

test('applyTraining: routineスロットのメッセージは「ルーチン構成（成功）: 演技構成 +N」形式', () => {
  const s = base();
  const r = DT.engine.applyTraining(s, ['routine'], () => 0.3);
  assert.strictEqual(r.messages[0], 'ルーチン構成（成功）: 演技構成 +3'); // routineGain2×1年目1.5倍でgain3
});
...
test('applyTraining: h1d習熟が20を超えた練習でv1d/d2の解禁が告知される（一度だけ）', () => {
  ...
  assert.ok(joined.includes('1DV') && joined.includes('解禁'), 'v1d解禁の告知が出る');
  assert.ok(joined.includes('2D') && joined.includes('解禁'), 'd2解禁の告知が出る');
