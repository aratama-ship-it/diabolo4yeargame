...
- **バランス権限（V3-4と同形式、≤5イテレーション、レポートに表を必須）**: ゲート廃止でスペシャスコア底上げ→S率上振れ濃厚。調整可能ノブ: LEVELS base/growth/sd、endingランク閾値、SLOTS.gridGain/routineGain。禁止: リマップ・配点・ミスモデル係数・エンジンパイプライン・断言緩和。目標: 卒業20/20、bestRank≤3、特別指導≥5/20、**S≤3/20**、E≤4/20、どのバケツも≤9/20（A/B帯にも1つ以上入ることを目指すが必達ではない→結果を報告）
- **世界大会キャリブレーション（ユーザー必達指定）**: 4年目の世界大会の場を、①AJDC優勝クラスのスコア（当年AJDC 1位相当、目安display 88〜93）が**6位以内**に入る ②**display 95以上なら3位以内圏**になる分布に。開始ノブ案: LEVELS.worlds base 63 / growth 3 / sd 13。検証方法: 4年目worlds場のスコア分布パーセンタイルをプローブし、rank6境界≒88〜91・rank3境界≒93〜96を確認して結果をレポートに記載（魁人の曲線66→73.5生値が場と整合することも確認）
